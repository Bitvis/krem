...
def run():
    ...
    if not result:
        if not os.path.isfile(os.path.join(temp_task_path, p.DEAFULT_TASK_SCRIPT)):
            result = rc.FAIL
            print("ERROR: Failed to deploy task template file: " + p.DEFAULT_TASK_SCRIPT)
        if not os.path.isfile(os.path.join(temp_task_path, p.DEAFULT_TASK_SETUP_FILE)):
            result = rc.FAIL
            print("ERROR: Failed to deploy task template file: " + p.DEAFULT_TASK_SETUP_FILE)
